...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
    ...
    public List<SysDepartTreeModel> queryMyDeptTreeList(String departIds) {
        ...
        Set<String> rootOrgCodes = new HashSet<>(Arrays.asList(codeArr));
        for (SysDepart dept : listDepts) {
            if (rootOrgCodes.contains(dept.getOrgCode())) {
    ...
    private void verifyOrgCategory(SysDepart sysDepart) {
        ...
        if (oConvertUtils.isNotEmpty(sysDepart.getOrgCategory()) && DepartCategoryEnum.DEPART_CATEGORY_POST.getValue().equals(sysDepart.getOrgCategory())) {
            long childCnt = this.count(new LambdaQueryWrapper<SysDepart>().eq(SysDepart::getParentId, sysDepart.getId()));
            if (childCnt > 0) {
        ...
        if (oConvertUtils.isNotEmpty(sysDepart.getOrgCategory()) && DepartCategoryEnum.DEPART_CATEGORY_SUB_COMPANY.getValue().equals(sysDepart.getOrgCategory())
            ...
            LambdaQueryWrapper<SysDepart> parentCheck = new LambdaQueryWrapper<>();
            parentCheck.eq(SysDepart::getId, sysDepart.getParentId());
            parentCheck.in(SysDepart::getOrgCategory, DepartCategoryEnum.DEPART_CATEGORY_POST.getValue(), DepartCategoryEnum.DEPART_CATEGORY_DEPART.getValue());
            long parentHit = this.count(parentCheck);
            if (parentHit > 0) {
        ...
        if (oConvertUtils.isNotEmpty(sysDepart.getOrgCategory()) && DepartCategoryEnum.DEPART_CATEGORY_DEPART.getValue().equals(sysDepart.getOrgCategory())) {
            LambdaQueryWrapper<SysDepart> childrenCheck = new LambdaQueryWrapper<>();
            childrenCheck.eq(SysDepart::getParentId, sysDepart.getId());
            childrenCheck.in(SysDepart::getOrgCategory, DepartCategoryEnum.DEPART_CATEGORY_COMPANY.getValue(), DepartCategoryEnum.DEPART_CATEGORY_SUB_COMPANY.getValue());
            long companyChildCnt = this.count(childrenCheck);
            if (companyChildCnt > 0) {
    ...
    public void deleteBatchWithChildren(List<String> ids) {
        ...
        for (String id : ids) {
            ...
            if (oConvertUtils.isNotEmpty(depart.getParentId())) {
                parentIdSet.add(depart.getParentId());
            }
        }
    ...
    public List<SysDepartTreeModel> searchByKeyWord(String keyWord, String myDeptSearch, String departIds, String orgCategory, String depIds) {
        ...
        if (StringUtils.isNotEmpty(myDeptSearch)) {
        ...
        if (!departList.isEmpty()) {
            for (SysDepart depart : departList) {
                SysDepartTreeModel model = new SysDepartTreeModel(depart);
    ...
    public List<SysDepart> queryUserDeparts(String userId) {
        ...
        if (CollectionUtils.isEmpty(sysDeparts)) {
            return sysDeparts == null ? Collections.emptyList() : sysDeparts;
        }
        sysDeparts.stream()
     ...
     * 获取负责部门父节点
     *
     * @param departRows 负责部门列表
     * @return 父级 orgCode，多值逗号分隔
     */
    private String getMyDeptParentNode(List<SysDepart> departRows) {
        Map<String, String> codeGroupMap = new HashMap<>(5);
        //1.先将同一公司归类
        for (SysDepart dept : departRows) {
            String code = dept.getOrgCode().substring(0, 3);
            if (codeGroupMap.containsKey(code)) {
                String mapCode = codeGroupMap.get(code) + SymbolConstant.COMMA + dept.getOrgCode();
                codeGroupMap.put(code, mapCode);
            } else {
                codeGroupMap.put(code, dept.getOrgCode());
            }
        }
        StringBuilder parentOrgCodeBuf = new StringBuilder();
        //2.获取同一公司的根节点
        for (String str : codeGroupMap.values()) {
            String[] arrStr = str.split(SymbolConstant.COMMA);
            parentOrgCodeBuf.append(SymbolConstant.COMMA).append(this.getMinLengthNode(arrStr));
        }
        return parentOrgCodeBuf.substring(1);
    }
    ...
    public List<SysDepartTreeModel> queryTreeListByPid(String parentId, String ids, String primaryKey) {
        Consumer<LambdaQueryWrapper<SysDepart>> applyTreeQueryScope = i -> {
            if (oConvertUtils.isNotEmpty(ids)) {
    ...
    public List<SysDepartTreeModel> queryDepartAndPostTreeSync(String parentId, String ids, String primaryKey,
        ...
        Consumer<LambdaQueryWrapper<SysDepart>> applySyncTreeQueryScope = i -> {
            if (oConvertUtils.isNotEmpty(ids)) {
                if (CommonConstant.DEPART_KEY_ORG_CODE.equals(primaryKey)) {
                    i.in(SysDepart::getOrgCode, Arrays.asList(ids.split(SymbolConstant.COMMA)));
                } else {
        ...
        lqw.eq(true, SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString());
        lqw.func(applySyncTreeQueryScope);
        lqw.orderByAsc(SysDepart::getDepartOrder);
        List<SysDepart> list = list(lqw);
        //设置用户id,让前台显示
        this.setUserIdsByDepList(list);
    ...
    public void getTreeByKeyWord(String keyWord, List<SysDepartTreeModel> allResult, List<SysDepartTreeModel> newResult) {
        ...
        for (SysDepartTreeModel model : allResult) {
            String departName = model.getDepartName();
            if (keyWord != null && departName != null && departName.contains(keyWord)) {
                newResult.add(model);
                continue;
            }
        ...
        for (String userId : userIdArray) {
            ...
            if (oConvertUtils.isNotEmpty(sysUser.getDepartIds())) {
                if (!sysUser.getDepartIds().contains(departId)) {
                    sysUser.setDepartIds(sysUser.getDepartIds() + SymbolConstant.COMMA + departId);
                }
            } else {
    ...
    private void updateChargeDepart(SysDepart sysDepart) {
        ...
        if (oConvertUtils.isEmpty(directorIds)) {
            ...
            //都不为空，需要比较，进行添加或删除（使用 Set 按用户 id 精确匹配，避免 String.contains 误判）
            Set<String> newDirectorSet = splitCommaIdsToSet(directorIds);
            Set<String> oldDirectorSet = splitCommaIdsToSet(oldDirectorIds);
            List<String> userIdList = Arrays.stream(oldDirectorIds.split(SymbolConstant.COMMA))
                    .map(String::trim)
                    .filter(id -> !id.isEmpty() && !newDirectorSet.contains(id))
                    .toList();
            for (String userId : userIdList) {
                this.deleteChargeDepId(departId, userId);
            }
            String addUserIds = Arrays.stream(directorIds.split(SymbolConstant.COMMA))
                    .map(String::trim)
                    .filter(id -> !id.isEmpty() && !oldDirectorSet.contains(id))
    ...
    /**
     * 将逗号分隔的用户 id 转为去重后的 Set，用于负责部门变更时的精确比对。
     */
    private static Set<String> splitCommaIdsToSet(String commaIds) {
        if (oConvertUtils.isEmpty(commaIds)) {
            return Collections.emptySet();
        }
        Set<String> set = new HashSet<>();
        for (String id : commaIds.split(SymbolConstant.COMMA)) {
    ...
    private void deleteChargeDepId(String departId, String userId) {
        ...
        for (SysUser sysUser : userList) {
            ...
            List<String> list = new ArrayList<>(Arrays.asList(departIds.split(SymbolConstant.COMMA)));
    ...
    private void findPath(ExportDepartVo departVo, List<String> path, Integer tenantId, List<ExportDepartVo> childrenDepart, List<String> departIdList) {
        ...
        for (ExportDepartVo exportDepartVo : departList) {
            //存放子级路径
    ...
    public void importExcel(List<ExportDepartVo> listSysDeparts, List<String> errorMessageList) {
        ...
        for (ExportDepartVo exportDepartVo : listSysDeparts) {
            ...
            try {
                ...
                //没有查找到parentDept，记录日志便于排查
                log.warn("导入部门失败，第{}行，部门名称：{}，错误信息：{}", num + 1, exportDepartVo.getDepartName(), e.getMessage());
            }
            //没有错误的时候才会导入数据
            if (izExport) {
    ...
    public void importSysDepart(List<SysDepartExportVo> listSysDeparts, List<String> errorMessageList) {
        ...
        for (SysDepartExportVo departExportVo : listSysDeparts) {
            ...
            try {
                ...
                //没有查找到parentDept，记录日志便于排查
                log.warn("导入系统部门失败，第" + (num + 1) + "行，部门名称：" + departExportVo.getDepartName() + "，错误信息：" + e.getMessage());
            }
            //没有错误的时候才会导入数据
            if (izExport) {
    ...
    private void getChildrenDepartPositionByOrgCode(List<SysPositionSelectTreeVo> selectTreeVos, Map<String, String> departNameMap, SysDepartPositionVo sysDepartPosition) {
        ...
        List<SysDepartPositionVo> positionList = baseMapper.getDepartPostByOrgCode(orgCode + SymbolConstant.PERCENT_SIGN);
    ...
    public static List<SysPositionSelectTreeVo> buildTree(List<SysPositionSelectTreeVo> nodes) {
        // 1. 去重：根据 ID 去重，保留首次出现的节点（单线程构建，使用 LinkedHashMap 保持插入顺序）
        Map<String, SysPositionSelectTreeVo> uniqueNodes = new LinkedHashMap<>();
    ...
    public void updateChangeDepart(SysChangeDepartVo changeDepartVo) {
        ...
        switch (dropPosition) {
            ...
            case 1:
                ...
                break;
            default:
                throw new JeecgBootBizTipException("无效的拖拽位置");
        }
        //5. 清空缓存
        redisUtil.removeAll(CommonConstant.DEPART_NAME_REDIS_KEY_PRE);
    }
    ...
    private void validateDragOperation(SysDepart dragDept, SysDepart targetDept, Integer dropPosition) {
        ...
        if (dragDept.getId().equals(targetDept.getId())) {
            throw new JeecgBootBizTipException("不能拖拽到自身");
        }
        // 禁止拖拽到自身子部门
        if (isDescendant(dragDept, targetDept.getId())) {
            throw new JeecgBootBizTipException("不能拖拽到自身子部门");
        }
        //公司岗位判断
    ...
    private void moveToAbove(SysDepart dragDept, SysDepart targetDept) {
        Objects.requireNonNull(targetDept, "目标部门不能为空");
        // 获取目标部门同级的所有部门
        List<SysDepart> siblings = departMapper.getDepartNoParent();
        // 更新被拖拽部门的排序值（置顶为 0，具体顺序由 computingSort 重算）
