 ...
 *   <li>Redis requestId 去重（集群安全，配合 $share 订阅）</li>
 *   <li>按设备查 STS 凭证缓存，命中则直接 MQTT 下发</li>
 *   <li>同设备转发节流 + inflight 防并发重复申请</li>
...
public class CollectUrlRequestHandler {
    ...
    public void handle(String deviceCode, String payload) {
        ...
        Optional<CollectUrlResponseCmd.StsParams> cached = credentialCache.getIfValid(deviceCode);
        if (cached.isPresent()) {
            respondFromCache(deviceCode, msg.getRequestId(), cached.get());
            return;
        }
        ...
        if (!credentialCache.tryAcquireInflight(deviceCode, msg.getRequestId())) {
            log.debug("[DataCollect] 跳过进行中 OSS 授权请求 deviceCode={} requestId={}",
                    deviceCode, msg.getRequestId());
            return;
        }
    ...
    private void respondFromCache(String deviceCode, String requestId,
                                  CollectUrlResponseCmd.StsParams params) {
        CollectUrlResponseCmd cmd = credentialCache.buildSuccessResponse(deviceCode, requestId, params);
        commandService.sendCollectUrlResponse(deviceCode, cmd);
        log.info("[DataCollect] OSS 凭证缓存命中，已直接下发 deviceCode={} requestId={}",
                deviceCode, requestId);
        String topic = "device/" + deviceCode + "/" + DataCollectConstant.MQTT_DOWN_COLLECT_URL_RESP;
        logService.recordLog(null, deviceCode,
                DeviceConstant.OperationType.DATA_COLLECT,
                "OSS 凭证缓存命中，已直接下发",
                OperationLogDetail.ofRequest(requestId, topic),
