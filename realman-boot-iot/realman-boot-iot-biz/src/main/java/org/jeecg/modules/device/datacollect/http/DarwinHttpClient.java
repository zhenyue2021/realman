...
public class DarwinHttpClient {
    ...
    public boolean replayOutbox(DarwinHttpOutbox outbox) {
        try {
            postRaw(outbox.getPath(), outbox.getRequestBody(), Void.class, outbox.getDeviceCode());
            return true;
        } catch (Exception e) {
    ...
    private <T> T postRaw(String path, String bodyJson, Class<T> responseType, String deviceCode) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        String apiKey = properties.getHttp().getApiKey();
        if (StringUtils.hasText(apiKey)) {
