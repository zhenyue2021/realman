...
| 下行 | `webrtc/{deviceCode}/command/start`          | 下发 WebRTC 开始指令（含房间号、信令服务器地址/密钥、TURN/STUN 服务器）；平台等待 ACK 最多 5 秒    | `WebRtcStartCommand`                                                               |
| 下行 | `webrtc/{deviceCode}/command/stop`           | 下发 WebRTC 停止指令（fire-and-forget，不等待 ACK）                            | `WebRtcStopCommand`                                                                |
| 上行 | `webrtc/{deviceCode}/command/start/ack`      | WebRTC 开始指令 ACK；`success=false` 或超时均视为开启失败                        | `WebRtcAck`                                                                        |
| 上行 | `webrtc/{deviceCode}/command/stop/ack`       | WebRTC 停止指令 ACK（平台仅记录日志，不阻塞流程）                                    | `WebRtcAck`                                                                        |

**订阅建议（机器人最小集）**：`device/{deviceCode}/config/push`、`.../command/+`、`.../ota/notify`、`.../camera/stream/query`、
`.../slam/upload/permit`、`.../slam/sync/command`、`.../ext-params/ack`、`webrtc/{deviceCode}/command/+`（可按能力裁剪）。
...
WebRTC 指令走独立的 `webrtc/{robotCode}/...` Topic 前缀，Payload 同样经 `CommandEncryptService` 加解密。
流程如下：
...
平台                                   机器人
  ...
  │<─ webrtc/{robotCode}/command/start/ack│  2. 机器人响应（5 秒超时，超时视为失败）
  │                                      │
  │   （WebRTC P2P 连接建立，视频流传输）  │
  │                                      │
  │── webrtc/{robotCode}/command/stop ──>│  3. 平台下发 WebRTC 停止指令（）
  │<─ webrtc/{robotCode}/command/stop/ack│  4. 机器人可选回应（平台仅记日志）
```

> **注意**：`webrtc/` Topic 中的 `{deviceCode}` 为**机器人**设备编码（`robotCode`），非主控编码。

**下行 `webrtc/{robotCode}/command/start`**
...
| `signalKey`   | 信令服务器访问密钥                      |
| `turnServers` | TURN 中继服务器列表；`url`                               |
| `stunServers` | STUN 服务器 IP 列表（字符串数组）                                          |
| `timestamp`   | 平台下发时间（毫秒）                                                      |

**上行 `webrtc/{robotCode}/command/start/ack`（成功）**
...
**上行 `webrtc/{robotCode}/command/start/ack`（失败）**
...
**下行 `webrtc/{robotCode}/command/stop`**
...
**上行 `webrtc/{robotCode}/command/stop/ack`（可选，平台仅记日志）**
...
1. 订阅 `webrtc/{robotCode}/command/+`，收到 `start` 后连接信令服务器建立 WebRTC 连接，完成后在 **5 秒内**回复 ACK。
2. `start/ack` 中 `success=false` 或未在 5 秒内应答，平台将终止本次遥操并提示操作员。
3. 收到 `stop` 后立即中断 WebRTC 连接，可选回复 `stop/ack`。
...
6. **外部服务参数**：订阅 `device/{code}/ext-params/ack`，发起请求时 `commandId` 须唯一；根据 `bjExpiration` 在过期前主动刷新，收到
   `code=400` 稍后重试。
7. **WebRTC**：订阅 `webrtc/{robotCode}/command/+`；`start/ack` 中 `commandId` 须与下行指令一致；必须在 **5 秒**内回复，否则平台报超时错误。`stop` 为 fire-and-forget，`stop/ack` 可选。
8. **调试**：可将 `device.encrypt.enabled=false` 用明文 JSON 抓包；上线前务必恢复为加密。
