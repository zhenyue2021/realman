# realman-boot 现场部署运维手册（完整版）

> 版本：v3.0 | 日期：2026-06-10  
> 适用对象：**零基础运维 / 技术支持**（未接触过本项目的同事也可按本文独立交付）  
...
0. [部署总览（零基础必读）](#0-部署总览零基础必读)
1. [使用说明与场景选择](#1-使用说明与场景选择)
2. [出发前信息收集](#2-出发前信息收集)
3. [环境与规格](#3-环境与规格)（含 [§3.4 云服务器专章](#34-云服务器部署专章)）
4. [云数据库准备（场景 A/B）](#4-云数据库准备场景-ab)
5. [本机 MySQL 准备（场景 C）](#5-本机-mysql-准备场景-c)
6. [服务器初始化](#6-服务器初始化)
7. [目录结构](#7-目录结构)
8. [完整配置文件](#8-完整配置文件)
9. [Docker Compose](#9-docker-compose)
10. [Dockerfile](#10-dockerfile)
11. [Nacos 生产配置](#11-nacos-生产配置)
12. [EMQX 初始化与一机一密](#12-emqx-初始化与一机一密)
13. [应用构建与镜像打包](#13-应用构建与镜像打包)
14. [标准部署流程](#14-标准部署流程)
15. [部署验证](#15-部署验证)
16. [回滚方案](#16-回滚方案)
17. [现场故障排查](#17-现场故障排查)
...
<!-- legacy section anchors preserved below for old links -->
### 14.1 写入全部配置
