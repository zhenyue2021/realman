...
> **v2.1 修订说明（2026-07-08）**：本文档第五章"设备通信中台"最初把 OTA PRD 第九章的设备侧 HTTP 接口理解为"设备协议从 MQTT 切到 HTTP"，评审后确认这是误读。**系统与设备之间的通信协议维持 MQTT 不变**（唯一例外是设备上电后、建立 MQTT 连接前的一次性 HTTP 自注册）；新增的 HTTP 能力是**通信中台对外统一输出的接口协议**（服务对外 API 出口 + 面向 SmartArm 等后续项目的可选设备接入协议），与"南向对设备"是两回事。第四、五章已按此修正，详见配套的两份详细设计文档。
...
- v1.0 只规划了 MQTT 一种协议；产品明确**系统与设备之间的通信协议维持 MQTT 不变**（设备上电自动注册接口除外），但需要新增一层**对外统一输出的 HTTP 接口协议**——服务把各自的能力经由通信中台统一以 REST 形式对外提供，同时为 SmartArm 等后续项目预留"设备直接走 HTTP 接入"的可选协议扩展点。OTA PRD 第九章定义的心跳/Token/进度推送接口契约，正是这层 HTTP 协议要承接的逻辑契约——现有 MQTT 设备通过协议适配在南向仍走 MQTT 报文，语义上对齐同一套契约。
...
| 设备通信 | 只有 MQTT 集成平台 | **设备通信中台**：南向仍是 MQTT 单协议（注册除外）；新增**北向 HTTP 统一对外网关**，服务对外 API 输出 + 未来 SmartArm 等 HTTP 原生设备接入 |
| OTA 与通信层关系 | OTA 进度经 MQTT 平台转发 MQ 事件 | 现有 master/slave 设备的 OTA 心跳/进度仍经 MQTT 上行，由通信中台归一化后转发给 OTA 平台；OTA PRD 第九章的 HTTP 契约作为北向网关的等价协议对外暴露（见 5.3 协议等价映射表）|
...
设备通信中台是**设备层与云端所有通信的唯一通道**，但"通道"要分南向、北向两侧看，不能笼统说"MQTT+HTTP 双协议对设备"：

- **南向（对设备）**：协议维持 **MQTT 一种**，与现状一致，不改变现有设备 SDK。唯一例外是**设备上电后、建立 MQTT 连接前的一次性 HTTP 自注册**（现状 `DeviceProvisionController` 的定位，V2 里完善为对齐设备基座的注册协议）。
- **北向（对外/服务间）**：新增 **HTTP 统一对外网关**，职责有两个，且互不冲突：
  1. 各业务服务（OTA、设备管理业务平台、数据处理、GLN）的对外 REST API，经由通信中台统一路由输出（而不是各自裸露端口对外），便于统一鉴权、限流、审计、第三方接入管理；
  2. 为 SmartArm 等**后续**不采用现有 MQTT SDK 的设备类型，预留一条可选的 HTTP 直连协议入口——这是一个面向新设备类型的**协议扩展点**，不是要求现有 master/slave 设备迁移协议。

OTA PRD 第九章定义的心跳/Token/进度推送/状态补传接口契约，在这个修正后的模型里理解为：**现有 MQTT 设备通过南向 MQTT Topic 传递同样语义的数据，由通信中台在内部归一化后，按同一套逻辑契约转发给 OTA 平台；这套契约同时以 HTTP 形式在北向网关对外暴露，供 SmartArm 或外部系统直接调用**。两侧共用同一套内部事件模型和业务处理逻辑，只是南向的传输介质不同。详见详细设计文档的"协议等价映射表"。

### 5.2 双侧接入层（示意）

```mermaid
flowchart LR
  subgraph south["南向：设备侧（协议不变）"]
    MQTTIN["MQTT 接入子模块\nEMQX Auth/ACL · Dispatcher · Publisher\n（心跳/OTA/SLAM/数采，均走 MQTT）"]
    PROVISION["HTTP 自注册（唯一例外）\n设备上电、MQTT 连接前"]
  end
  subgraph north["北向：对外网关（新增）"]
...
- **MQTT 接入子模块**：原样承接 v1.0 已规划的 `MqttAuthController`/`MqttMessageDispatcher`/`MqttPublisher`/`RedisPendingListenerConfig`（集群 ACK 协调），是现有 master/slave 及 SLAM/WebRTC/数采指令的唯一南向通道，架构上不改变。
- **HTTP 统一对外网关（新增）**：不是设备协议的替代品，而是（a）各业务服务对外 API 的统一出口，（b）未来 HTTP 原生设备（SmartArm）的可选接入协议适配层。与面向 Web 管理端的通用 `realman-gateway`（Spring Cloud Gateway）职责不同——后者服务前端管理台的通用业务 API，通信中台北向网关专注"设备域"相关能力的对外输出，两者边界见详细设计文档。
- **统一上行事件模型**：无论数据来自 MQTT 报文体、HTTP 自注册请求，还是北向网关收到的 HTTP 请求，接入层都归一化为同一个 `DeviceUplinkEvent`，下游路由规则、审计埋点、状态监控埋点只写一套逻辑。

### 5.3 路由注册表（更新，南北向分列）

| 来源 | 匹配规则 | 目标 | 传输方式 |
| --- | --- | --- | --- |
| 南向 MQTT `device/{code}/ota/*` | 现有 master/slave 的 OTA 心跳/进度/状态补传 | OTA 平台 | 内部事件归一化后 Feign 转发 |
| 南向 HTTP（唯一例外）`POST /internal/device/provision` | 设备上电自注册 | 设备基座（设备管理业务平台）| 直接处理 |
| 南向 MQTT `device/{code}/slam/*` | GLN 遥操 | IoT 业务服务 | 内部事件 |
| 南向 MQTT `device/{code}/datacollect/*` | 数采指令/OSS 回传 | 数据处理模块 | **HTTP 直连（见第六章，替代原 RocketMQ）** |
| 南向 MQTT `$SYS/.../connected|disconnected` | 设备上下线 | 设备基座（online-event）| 内部事件 |
| 北向 HTTP `/api/v1/ota/**`、`/api/v1/devices/**` 等 | 外部系统 / 未来 HTTP 原生设备（SmartArm）调用同一套逻辑契约 | 对应后端服务（OTA/设备基座）| 网关反向代理 |
...
| 南向 MQTT 上行 → 业务处理（心跳/OTA/SLAM/WebRTC）| 内部归一化后同步转发 | 状态机需要立刻可查，不适合异步排队 |
| 北向 HTTP 请求（外部系统 / 未来 HTTP 设备）| 同步处理并即时响应 | 与南向共用同一套内部处理逻辑，只是入口协议不同 |
...
沿用业务架构 v1.2 对通信中台的定位——大文件传输、实时操控与视频通道也归口在这里（WebRTC 信令、断点续传的底层能力），均走南向 MQTT/专用通道，不受本次北向 HTTP 网关新增影响。
...
设备通信中台是能力总线之下"设备侧能力"这一类目的具体实现者；能力总线本身不关心设备协议细节，只关心"业务应用如何发现、调用、鉴权访问通信中台暴露的契约"。通信中台的北向 HTTP 网关（见[通信中台详细设计](./2026-07-08-device-comm-hub-detailed-design.md)第四章）正是"设备域能力"通过能力总线对外/对第三方输出的具体承载——**通信中台是被总线治理的众多底座服务之一**，两者不是同一层。
...
| 模块 | 调整点 |
| --- | --- |
| **OTA 平台** | 设备侧协议维持 MQTT，不是切到 HTTP；OTA PRD 第九章的心跳/Token/进度推送契约通过通信中台的"协议等价映射表"（[详见](./2026-07-08-device-comm-hub-detailed-design.md#五统一上行事件模型与协议等价映射表核心交付)）以 `DeviceUplinkEvent` 形式消费，OTA 平台只需实现一套业务逻辑，不用区分协议来源。OTA 自身的管理端 API（固件管理、任务管理）经通信中台北向网关统一对外输出。`is_test_device` 等前置校验字段一律只读设备基座 SSOT，不自建设备表。|
| **数据处理模块（Darwin）** | 与通信中台之间走服务间直连 HTTP（第六章），这与"南向 MQTT 不变"无关——是两个后端服务的相互调用，不经过设备协议适配层，也不经过北向对外网关。 |
| **GLN（遥操）/ 任务规划** | 遥操指令、视频信令、SLAM 数据继续走通信中台南向 MQTT，不受本次修订影响；下发任务前对设备状态（在线态/四态/主控-机器人绑定）的查询一律走设备基座 SSOT 只读接口，不再各自维护设备表或直连设备确认状态。 |
| **状态监控** | 作为横切能力，统一订阅通信中台归一化后的 `DeviceUplinkEvent` 与设备基座的操作审计事件，不需要单独适配 MQTT/HTTP 两种协议——协议差异已经在通信中台内部被抹平。 |
| **平台能力总线** | 治理层设计不变（第七章）；新增的强调点是"设备域"能力的对外契约输出具体由通信中台北向网关承载，总线的其余部分（Nacos 注册、`*-contract` 模块、契约版本治理）覆盖全部业务应用与底座之间的调用，不只是设备相关的。 |
...
| Phase 1 OTA 平台独立 + 能力对齐 | OTA 成为独立平台，**设备侧协议维持 MQTT**，按 PRD 补齐能力 | 迁移固件/任务/状态机代码到新服务；**重建 15 态状态机、密钥生命周期、批量升级策略、错误码体系**（对齐 PRD，而不是照搬现有 8 态）；心跳/进度/Token 等语义映射为 MQTT Topic | 6-8 周（比 v1.0 估算的 4 周更长，因为要补的能力远超"搬家"）|
| Phase 1.5（新增）数据处理 HTTP 化 | 退役 RocketMQ 桥接 | 按第六章方案双写过渡、灰度、下线 | 2-3 周，可与 Phase 1 并行（依赖对象不同）|
| Phase 2 设备通信中台独立（南向 MQTT 不变 + 新增北向 HTTP 网关） | 通信层独立，新增对外统一 HTTP 网关 | 迁移 MQTT 相关代码（南向不变）；新增北向 HTTP 网关子模块（服务对外 API 输出 + 为 SmartArm 预留协议扩展点）；建协议等价映射表与路由注册表 | 5-6 周（比 v1.0 估算的 4 周增加 HTTP 网关工作量）|
...
| 决策点 | 结论 | 理由 |
| --- | --- | --- |
| 现有设备（master/slave）是否切换到 HTTP 协议 | **否**，维持 MQTT，仅上电自注册用 HTTP | 避免现有设备 SDK 大改；HTTP 是新增的北向能力，不是南向协议替代 |
| OTA PRD 第九章的 HTTP 接口契约放在哪里 | 作为通信中台北向网关对外暴露的逻辑契约；现有 MQTT 设备通过协议等价映射复用同一套内部处理逻辑 | 一套契约、两种传输介质，避免 OTA 平台自己再维护一套"MQTT 版"和"HTTP 版"两份业务逻辑 |
...
| MQTT 是否完全废弃 | 否，是南向唯一协议，长期保留 | 现有设备 SDK、SLAM/WebRTC 等强实时场景均走 MQTT；HTTP 只是北向新增能力 |
