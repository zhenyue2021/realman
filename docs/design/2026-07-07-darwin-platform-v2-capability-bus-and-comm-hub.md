...
- v1.0 只规划了 MQTT 一种协议；产品明确**系统与设备之间的通信协议维持 MQTT 一种，包括 SmartArm 等第三方设备在内，都不例外**（设备上电自动注册接口除外）。新增的是一层**对外统一输出的 HTTP 接口协议**，但它不是给设备用的第二种协议，而是服务两件事：① 各业务服务经通信中台统一以 REST 形式对外提供业务/管理类 API；② **HTTP-MQTT 桥接**——让 SmartArm 等第三方的**业务后台系统**（不是设备本身）能用标准 HTTP 请求/响应与设备实时交互（下指令、收数据），由通信中台内部转译为 MQTT 发布/订阅，第三方无需自行实现 MQTT 客户端。OTA PRD 第九章定义的心跳/Token/进度推送接口契约，实际由现有 MQTT 设备通过 MQTT Topic 承载，通信中台归一化后既转发给 OTA 平台，也可经桥接暴露给需要的第三方。
...
| 设备通信协议 | 下发走 MQTT publish（`device/{code}/ota/notify`）；进度经 `OtaProgressHandler` 订阅 MQTT 得到 | 现有 master/slave 及未来 SmartArm 等设备协议统一维持 MQTT；需要把 PRD 第九章的心跳/Token/进度推送/状态补传语义映射为对应 MQTT Topic；通信中台 WEB 端向网关不做"设备协议"，而是提供 HTTP-MQTT 桥接（同步下行 + Webhook/轮询上行），供第三方业务后台在不接触 MQTT 的前提下与设备交互 | 需要落地 **HTTP-MQTT 桥接**（而不是把现有/未来设备直接切到 HTTP 协议），详见[通信中台详细设计](./2026-07-08-device-comm-hub-detailed-design.md)第四、五章 |
...
flowchart TB
  ...
  subgraph devices["L3 设备层（统一 MQTT，注册除外）"]
    MASTER["遥操设备 master\n（MQTT，注册用 HTTP）"]
    SLAVE["机器人 slave\n（MQTT，注册用 HTTP）"]
    SMARTDEV["SmartArm 等第三方设备\n（同样 MQTT，注册用 HTTP）"]
  end

  subgraph north["WEB 端向消费方（业务/管理 API + HTTP-MQTT 桥接）"]
    WEBADMIN["Web 管理端"]
    THIRDBIZ["第三方业务后台\n（SmartArm 等，经桥接与设备交互）"]
  end
  ...
  CH <-->|MQTT，注册用 HTTP| SMARTDEV
  WEBADMIN -->|HTTP，业务/管理 API| CH
  THIRDBIZ -->|HTTP，桥接下行 + Webhook 上行| CH
...
| 设备通信 | 只有 MQTT 集成平台 | **设备通信中台**：设备端向仍是 MQTT 单协议（注册除外，含 SmartArm 等第三方设备）；新增 **WEB 端向 HTTP 统一对外网关**，提供业务/管理 API 输出 + HTTP-MQTT 桥接（供第三方业务后台不接触 MQTT 即可与设备交互）|
...
- **WEB 端向（对外/服务间）**：新增 **HTTP 统一对外网关**，职责有两个，且互不冲突：① 各业务服务（OTA、设备管理业务平台等）的对外 REST API 经此统一路由输出（而不是各自裸露端口），供设备信息查询、注册管理等业务/管理场景使用；② **HTTP-MQTT 桥接**——供 SmartArm 等第三方**业务后台**通过 HTTP 请求/响应与设备实时交互（下指令、收数据），通信中台内部转译为 MQTT 发布/订阅，第三方无需自行接入 MQTT（详见[通信中台详细设计](./2026-07-08-device-comm-hub-detailed-design.md)第四章）。这两个用途都不涉及"设备改用 HTTP"——设备本身，包括 SmartArm，仍统一走 MQTT。
...
| ② OSS 文件地址上报 | 机器人→通信中台(MQTT)→`MQ_TOPIC_FILE_REPORT`→数据处理 | 通信中台收到 MQTT 上报后调用 `POST /internal/data-processing/file-report`；属于事件型请求，失败写入 HTTP outbox 后后台补偿 |
| ③ 设备上下线状态推送 | `DeviceOnlineOfflineHandler`→`MQ_TOPIC_DEVICE_STATUS`→数据处理 | 通信中台在处理 `online-event` 时异步 HTTP 推送；属于事件型请求，失败写入 HTTP outbox 后后台补偿（允许短暂延迟）|
...
### 6.2.1 HTTP 替代 RocketMQ 后的可靠性等级

HTTP 直连不等于所有链路都降级为“调用失败即丢弃”。按业务是否需要同步响应，可靠性等级明确如下：

| 链路 | 同步语义 | 可靠性等级 | 补偿策略 |
| --- | --- | --- | --- |
| 采集授权 `requestOssAuth` | 必须在当前请求链路拿到 STS 凭证并下发给机器人 | 必须同步返回，不做后台补偿 | HTTP 失败立即释放 inflight 并通知机器人失败；不能写 outbox 后稍后补发，否则设备侧已超时/请求上下文已失效 |
| OSS 文件地址上报 | 事件型通知，对端处理可延迟 | 至少一次投递 | 首次 HTTP 失败写入 `darwin_http_outbox`，定时任务按退避策略重试；对端需按业务唯一键幂等 |
| 设备上下线状态推送 | 事件型通知，允许短暂延迟 | 至少一次投递 | 同上写 outbox 后后台补偿；状态类数据允许最终一致 |
| 工单创建、采集结果入站（Darwin → 我方） | 对端调用我方 HTTP | 至少一次由调用方保障，我方接口需幂等 | 我方继续保留 `darwin_workorder_mapping` 唯一键和文件上报 Redis 去重，避免对端重试造成重复写入 |
| 非关键审计/指标类旁路通知（未来新增） | 不影响业务闭环 | 尽力而为 | 可只记录日志与指标，不进入 outbox；新增前需在契约评审中明确标注 |

因此，`DarwinHttpClient.post` 需要区分两类请求：

1. **事件型、可后台补偿**：文件地址上报、设备状态推送等。失败写入或更新 outbox，状态为 `PENDING`/`FAILED_RETRYABLE`，由 `darwinHttpOutboxRetryJob` 扫描 `next_retry_at` 到期记录并重放原始 `request_body`。
2. **同步等待响应**：`requestOssAuth` 这类调用的返回值直接决定当前 MQTT 响应能否下发。失败只能按同步失败处理（释放 inflight、通知设备失败），不得进入 outbox，避免几分钟后补偿成功却无人消费响应的语义混乱。

### 6.3 需要下线/改造的现有代码

**已实现（与本节原计划有出入，见下）**：实现时发现 `device.datacollect` 包（含 MQTT 上行入口 `CollectUrlRequestHandler`/`OssAddressReportHandler`）仍然完整留在 `realman-boot-iot` 里，尚未迁移到通信中台（`realman-boot-comm-hub` 的 `MqttMessageDispatcher` 明确把 `datacollect/*` Topic 标注为"预留但本轮不处理"）。因此本次落地没有先做"把 datacollect 搬到通信中台"这个更大的前置迁移，而是**原地**在 `realman-boot-iot` 内给现有 Producer/Consumer 加上双路径开关，功能对等地实现了"退役 RocketMQ"这一诉求，两件事解耦，不互相阻塞：

| 现有文件（`device.datacollect` 包）| 处理方式（实际） |
| --- | --- |
| `producer/*`（`OssAuthRequestProducer`/`FileAddressReportProducer`/`DeviceStatusProducer`）| **未删除/未搬迁**，改为在方法内部按 `darwin.integration.http-enabled` 分支：默认走原 RocketMQ 逻辑不变；开关打开后改调用新增的 `DarwinHttpClient`（`org.jeecg.modules.device.datacollect.http`）|
| `consumer/OssAuthResponseConsumer` | 未删除（MQ 路径下仍工作）；HTTP 路径下这一步被折叠进 `OssAuthRequestProducer` 内部（HTTP 是同步往返，不再需要独立的异步响应消费者）|
| `consumer/WorkOrderCreateConsumer`/`FileReportConsumer` | 未删除（MQ 路径下仍工作）；新增 `DarwinIntegrationController`（`realman-boot-iot-api`）暴露 `POST /internal/task/data-collect-task`、`POST /internal/data-processing/collected-file` 两个对等的 HTTP 入口，供达尔文平台按 HTTP 方式调用；相比原 Consumer 逐项 try/catch 返回结果，不再因单项失败导致整批消息重投 |
| `MqSendHelper`/`MqMessageLogService`/`IotMqMessageLog` | MQ 路径继续复用；HTTP 成功与不可补偿的同步失败仍可写 `iot_mq_message_log` 审计。事件型 HTTP 失败不再只写日志，而是写入 `darwin_http_outbox`（`path`/`request_body`/`device_code`/`status`/`attempt_count`/`next_retry_at`/`last_error`/`created_at`），由 `darwinHttpOutboxRetryJob` 扫描 `PENDING`/`FAILED_RETRYABLE` 后按指数退避重试 |
| `DataCollectConstant` 中的 `MQ_TOPIC_*`/`MQ_TAG_*`/`MQ_GROUP_*` 常量 | **未标记 Deprecated**：MQ 路径仍是默认路径且完整可用，只有确认 HTTP 路径灰度通过、准备下线 RocketMQ 时才适合标记 |
| 发现的第 5 条既有链路 `FileReportConsumer`（`MQ_TOPIC_FILE_REPORT_IN`/达尔文→我方文件上报结果）| 本节原始四条链路枚举未包含此链路，实现时一并给出 HTTP 对应端点（见上表） |
| 达尔文平台真实 HTTP 契约 | **部分仍待联调**，`DarwinHttpClient`/`DarwinIntegrationController` 均按本文档假设路径实现（`POST {baseUrl}/internal/data-processing/oss-auth` 等）；入站鉴权已按 6.4 的最终约定落地，正式启用 `http-enabled` 前仍需要双方联调核实路径/字段/错误码 |
| `docs/design/2026-04-27-darwin-rocketmq-integration.md` | 仍保留作为历史记录，暂未加"已被取代"提示——因为 MQ 路径目前仍是默认生产路径，尚不适合宣称已被取代 |


### 6.4 入站 HTTP 最终认证约定

达尔文平台调用我方 `DarwinIntegrationController` 暴露的入站端点（当前为 `POST /internal/task/data-collect-task` 与 `POST /internal/data-processing/collected-file`）时，必须按以下同一规则认证；我方在进入 `createDataCollectTask`/`reportCollectedFile` 的业务处理前完成校验，失败时统一返回业务错误码 `401001`，不会创建/更新工单，也不会写入文件上报关联。

1. **配置项**：我方通过 `darwin.integration.inbound.*` 管理入站凭据，包含 `api-key-header`（默认 `X-Darwin-Api-Key`）、`api-key`、`hmac-secret`、`timestamp-header`（默认 `X-Darwin-Timestamp`）、`signature-header`（默认 `X-Darwin-Signature`）与 `timestamp-window-seconds`（默认 300 秒）。
2. **API Key 模式**：若配置了 `darwin.integration.inbound.api-key`，达尔文平台侧必须在 `api-key-header` 指定的请求头中传同一 API Key；我方使用常量时间比较，匹配则放行。
3. **HMAC 模式**：若配置了 `darwin.integration.inbound.hmac-secret`，达尔文平台侧必须同时传 `timestamp-header` 与 `signature-header`。时间戳采用 Unix epoch milliseconds；我方按 `timestamp-window-seconds` 限制时间窗口，超过窗口的请求按重放风险拒绝。
4. **签名串**：HMAC 签名内容固定为四行，以换行符 `\n` 拼接：`HTTP_METHOD`、`REQUEST_URI`、`TIMESTAMP`、`SHA256_HEX(RAW_REQUEST_BODY_UTF8)`。签名算法为 `hex(HMAC-SHA256(canonicalString, hmacSecret))`，其中 `REQUEST_URI` 使用不含域名的路径（例如 `/internal/task/data-collect-task`），请求体摘要必须基于原始 HTTP body 的 UTF-8 字节计算。
5. **兼容策略**：API Key 与 HMAC 可同时配置，任一方式校验通过即可放行；生产环境建议优先启用 HMAC，API Key 仅作为过渡或内网低风险场景的兜底。若两种凭据均未配置，我方默认拒绝入站调用，避免 `@IgnoreAuth` 端点裸奔。
6. **达尔文平台侧要求**：达尔文平台必须按上述规则生成请求头、签名和时间戳，并保证重试时重新计算时间戳/签名；双方联调时需固定相同的 body 序列化结果，避免因 JSON 空格、字段顺序或字符编码差异导致 body 摘要不一致。
...
1. **已完成**（落地位置是 `realman-boot-iot` 而非通信中台，见 6.3 说明）：新增 `DarwinHttpClient` HTTP 直连客户端，与现有 RocketMQ Producer 并行运行，功能开关 `darwin.integration.http-enabled`（默认 `false`）控制走哪条路径；`DarwinIntegrationController` 暴露对等的入站 HTTP 端点。
2. **未开始**：灰度验证 HTTP 路径的正确性与延迟指标（对比 MQ 路径的现有 Micrometer 指标）——需要达尔文平台侧先确认/对接真实 HTTP 契约后才能实际验证，当前的假设路径/字段尚未经过联调。
3. **未开始**：验证通过后关闭 RocketMQ 路径开关，观察一个完整业务周期无异常。
4. **未开始**：下线 RocketMQ 相关生产者/消费者代码与独立 Broker 部署（若该 Broker 没有其他用途）。
...
| 模块 | 调整点 |
| --- | --- |
| **OTA 平台** | 设备侧协议维持 MQTT（含未来 SmartArm 等设备），不因对接第三方而引入第二套协议；OTA PRD 第九章的心跳/Token/进度推送契约通过通信中台的"HTTP-MQTT 映射表"（[详见](./2026-07-08-device-comm-hub-detailed-design.md#五统一上行事件模型与http-mqtt-映射表核心交付)）以 `DeviceUplinkEvent` 形式消费，OTA 平台只需实现一套业务逻辑。OTA 自身的管理端 API（固件管理、任务管理）经通信中台 WEB 端向网关统一对外输出；若第三方需要下发升级指令/订阅进度，走 4.3 节的 HTTP-MQTT 桥接，不改 OTA 内部实现。`is_test_device` 等前置校验字段一律只读设备基座 SSOT，不自建设备表。|
...
| 阶段 | 目标 | 关键动作 | 周期 |
| --- | --- | --- | --- |
| Phase 0 契约先行（**已启动**）| 定规矩 | 新建 `*-contract` 模块（`realman-boot-device-info-contract`/`realman-boot-device-mgmt-contract`/`realman-boot-comm-hub-contract` 已建立，OTA contract 待 OTA 详细设计产出后补）；能力清单文档已产出；Gateway 路由与 Nacos 配置模板预留待后续 Phase | 1 周 |
| Phase 1 设备通信中台独立 | 通信层独立，设备端向 MQTT 不变 + 新增 WEB 端向 HTTP 网关 | 迁移 MQTT 相关代码（设备端向不变）；新增 WEB 端向 HTTP 网关子模块（业务/管理 API 输出 + HTTP-MQTT 桥接，供 SmartArm 等第三方业务后台使用）；建 HTTP-MQTT 映射表与路由注册表 | 2 周 |
...
| Phase 4 数据处理 HTTP 化 | 退役 RocketMQ 桥接 | **双写过渡的第 1 步已完成**（`DarwinHttpClient`+`DarwinIntegrationController`，默认关闭，见第六章 6.6）；灰度验证/关闭 MQ 路径/下线仍未开始，需达尔文平台侧先确认真实 HTTP 契约 | 2 周 |
...
| 决策点 | 结论 | 理由 |
| --- | --- | --- |
| 设备（含 SmartArm 等第三方设备）是否支持 HTTP 协议 | **否**，所有设备统一走 MQTT；唯一的 HTTP 是设备上电自注册这一次性引导请求 | 避免为不同厂商设备各维护一套协议实现；存量设备 SDK 不支持自注册的可由设备管理平台走手动注册兼容，新版本 SDK 需支持自注册 |
| SmartArm 等第三方如何与设备交互 | 不给第三方设备本身接入 HTTP，而是给第三方**业务后台**提供 HTTP-MQTT 桥接（同步下行 `publish-and-wait` + Webhook/轮询上行），内部转译为 MQTT | 第三方后台无需实现 MQTT 客户端即可下指令、收数据；设备侧协议保持单一，不因对接第三方而复杂化 |
| OTA PRD 第九章的 HTTP 接口契约放在哪里 | 现有 MQTT 设备通过 MQTT Topic 承载同等语义；契约本身不再对外暴露为"设备用 HTTP 接口"，而是通过通信中台的桥接机制服务需要的第三方 | 一套业务逻辑（消费 `DeviceUplinkEvent`），避免 OTA 平台为"MQTT 设备"和"HTTP 设备"各维护一套状态机 |
...
- [x] 能力清单（[已产出](./capability-catalog.md)）
- [x] OTA 平台详细设计（[已产出](./2026-07-09-ota-platform-detailed-design.md)，已对照 PRD 原文核对：完整 15 态状态机、30 个错误码、17 项系统设置均已转译；核心发现——PRD 的设备注册/Token/测试标记能力已在设备基座落地，OTA 平台直接复用，不重复实现）
- [x] 设备管理数据迁移方案（部分完成）：`iot_device` → `device_info`/`device_credential` 的一次性迁移工具已实现（`DeviceMigrationServiceImpl` + `POST /api/v1/admin/devices/migrate-from-legacy-iot`，默认关闭，需 `realman.migration.legacy-iot.enabled=true` 且二次确认文本），存量租户统一映射到 `tenant_id=1000`，旧 `device_secret` 按现值哈希落库（未强制轮换）。**范围限制**：`iot_device_auth`（主控-机器人授权配对）到 `device_binding` 的映射未纳入本轮，语义不完全对应，需产品另行确认规则。
- [ ] 数据处理 HTTP 直连接口契约（`*-contract` 模块，双方评审）——代码侧已按假设契约实现 `DarwinHttpClient`/`DarwinIntegrationController`（见第六章），但尚未与达尔文平台侧联调核实真实路径/字段/鉴权方式，本项仍待双方评审确认
