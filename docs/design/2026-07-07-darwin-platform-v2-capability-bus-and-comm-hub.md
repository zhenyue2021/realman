...
| `MqSendHelper`/`MqMessageLogService`/`IotMqMessageLog` | **复用而非改造**：`DarwinHttpClient` 直接复用同一张 `iot_mq_message_log` 表记录 HTTP 调用（`topic` 列固定填 `"HTTP"`，`tag` 列填请求路径），未新建独立的 HTTP 审计表 |
