...
| 达尔文平台真实 HTTP 契约 | **未知**，`DarwinHttpClient`/`DarwinIntegrationController` 均按本文档假设路径实现（`POST {baseUrl}/internal/data-processing/oss-auth` 等），尚未与达尔文平台侧对接确认，正式启用 `http-enabled` 前需要联调核实 |
...
| Phase 4 数据处理 HTTP 化 | 退役 RocketMQ 桥接 | **双写过渡的第 1 步已完成**（`DarwinHttpClient`+`DarwinIntegrationController`，默认关闭，见第六章 6.5）；灰度验证/关闭 MQ 路径/下线仍未开始，需达尔文平台侧先确认真实 HTTP 契约 | 2 周 |
