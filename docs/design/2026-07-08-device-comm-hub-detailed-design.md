...
| 请求体 | `{ topicSuffix: string, payload: object, waitAck: boolean, ackTimeoutMs?: number, ackTopicSuffix?: string, ackCommandIdField?: string }`（`topicSuffix` 如 `"command/restart"`，实际发布到 `device/{code}/{topicSuffix}`；ACK 参数默认 `bridge-ack` / `commandId`）|
| 处理流程 | ① 校验调用方对该设备/租户的授权范围；② 通信中台调用 `MqttPublisher` 向设备发布（按需 AES 加密，复用 `CommandEncryptService`）；③ `waitAck=false` 时立即返回 `{status: "PUBLISHED"}`；`waitAck=true` 时注册 pending，并记录期望的 ACK Topic 后缀与 ACK JSON 指令关联字段；④ 设备向约定 ACK Topic 上报 JSON 后，通信中台按该 Topic 对应的候选字段提取指令 ID，经 Redis Pub/Sub 唤醒等待方，超时返回 504 |
...
**桥接 ACK 契约**：默认情况下，设备必须向 `device/{deviceCode}/bridge-ack` 上报 JSON ACK，且 JSON 内必须包含与下行 payload 中相同的 `commandId`，示例：`{"commandId":"<下行commandId>","code":0,"message":"OK"}`。不同设备族如需使用差异化 ACK Topic 或字段名，可在下行请求中通过 `ackTopicSuffix` / `ackCommandIdField` 指定；对应上行 Topic 必须在 `comm_hub_topic_route` 初始化或管理端配置为 `route_type=BRIDGE_ACK`。自定义 ACK Topic 后缀采用版本化约定（推荐 `bridge-ack/v2`、`bridge-ack/v3`），不得改变既有 `bridge-ack` 的默认 `commandId` 语义。
