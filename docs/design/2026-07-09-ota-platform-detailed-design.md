...
| `POST /api/v1/devices/{id}/resource-probe`（9.7.4，operate=5，OTA 主动探测）| 下行（OTA 发起）+ 上行（设备回执）| 通信中台统一下行发布 `topicSuffix="ota/resource-probe"`（**本文档新增 Topic，非 PRD 已有映射**，见下方 2.1），`waitAck=true`，**已实现**（`OtaPrecheckServiceImpl#probeResourcesActively`，此前只补了 Topic 常量未接入实际调用，是真实遗漏）| 不产生 `DeviceUplinkEvent`（属于同步 publish-and-wait，不是异步上行事件）|
...
### 2.1 新增 Topic：`ota/resource-probe`（已实现）
...
`OtaPrecheckServiceImpl#checkResources` 已按此接入：主动探测成功（`status=ACKED`）时以回执数据为准且不再做心跳新鲜度校验（探测本身即"此刻"数据）；超时/异常/设备不支持时静默回退到心跳快照并保留原有 `disk_valid_seconds`/`power_valid_seconds`/`memory_valid_seconds`/`network_valid_seconds` 新鲜度校验。通信中台侧无需改动——`bridge-ack` 的 ACK 完成逻辑本就与具体业务 Topic 无关，任何 `waitAck=true` 的下行发布都能正确收到回执。
